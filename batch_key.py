...
def version():
    ...
    print("\033[0;33m[ Batch_Linux 1.1.0 - Language: Python3 ]\033[0m")


class process_expect:
    ...
    def loop_expect(hosts_model, file_cmd):
        ...
        expect.Pexpect.run_expect(hosts_model, file_cmd)
    ...
    if Path(f"{file_cmd}").exists():
        ...
        class multiprocess(threading.Thread):
            ...
            def run(self):
                ...
                proc = Process(target=process_expect.loop_expect,
                ...
                print(
                    f"\033[0;33m[ Applying parameters in {hosts_model} - Initialized | Pid: {pid} ]\033[0m")

                proc.join()

                print(
                    f"\033[0;32m[ Applying parameters in {hosts_model} - Finished | Pid: {pid} ]\033[0m")
...
def switch():
    ...
    elif sys.argv[1] == "--exec_c9200":

        key_model = "key_cisco_9200"
        hosts_model = hosts.C9200
        file_cmd = sys.argv[2]
        main(key_model, hosts_model, file_cmd)
